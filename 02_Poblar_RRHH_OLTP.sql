...
SELECT 
  eb.empleado_id, eb.nombre, eb.apellidos, eb.genero, eb.fecha_nacimiento, eb.estado_civil, eb.email, eb.telefono, eb.fecha_contratacion,
  round((p.salario_minimo + random()*(p.salario_maximo - p.salario_minimo))::numeric, 2) AS salario_actual,
  eb.departamento_id, eb.puesto_id, eb.codigo_oficina
FROM emp_base eb
JOIN oltp.puestos p ON p.puesto_id = eb.puesto_id;
...
WITH base_evaluaciones AS (
    SELECT empleado_id FROM oltp.empleados
    UNION ALL
    (SELECT empleado_id FROM oltp.empleados ORDER BY random() LIMIT 25) -- <-- ¡Paréntesis agregados aquí!
)
INSERT INTO oltp.evaluaciones (empleado_id, evaluador_id, fecha_evaluacion, calificacion, comentarios)
SELECT
  b.empleado_id,
  (SELECT empleado_id FROM oltp.empleados WHERE empleado_id <> b.empleado_id ORDER BY random() LIMIT 1),
  (DATE '2023-01-01' + (floor(random()*730))::int),
  round((1 + random()*4)::numeric,2),
  'Evaluación de desempeño anual'
FROM base_evaluaciones b;
